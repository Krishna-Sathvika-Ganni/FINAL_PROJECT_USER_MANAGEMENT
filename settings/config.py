...
class Settings(BaseSettings):
    ...
    database_url: str = Field(default='postgresql+asyncpg://user:password@postgres/myappdb', description="Database connection URL")
    ...
    # Email settings
    send_real_mail: bool = Field(default=False, description="Whether to send real emails or mock them")
    ...
    minio_use_ssl: bool = Field(default=False, description="Use SSL with Minio")
    minio_bucket_name: str = Field(default="picture", description="Minio bucket name")
    minio_base_url: str = Field(default="http://localhost:9000", description="Public base URL for Minio (used in profile picture URLs)")
