...
# Fetch user by ID (exists)
async def test_get_by_id_user_exists(db_session, user):
    retrieved_user = await UserService.get_by_id(db_session, user.id)
    assert retrieved_user.id == user.id

# Fetch user by ID (does not exist)
...
# Fetch user by nickname (does not exist)
...
# Fetch user by email (does not exist)
...
# Account lock after max failed logins
...
async def test_register_no_duplicate_email(db_session, email_service): 
    dummy_data = {
        "email": "test_real_email_unique@gmail.com",
...
# Register fails on duplicate nickname
...
# Register user with no nickname (should auto-generate)
async def test_register_no_nickname(db_session, email_service): 
    dummy_data = {
        "email": "test_real_email_nonick@gmail.com",
        "password": "TestPass123!",
        "role": UserRole.ADMIN.name
    }
    user = await UserService.register_user(db_session, dummy_data, email_service)
